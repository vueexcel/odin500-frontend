import { useContext, useState } from 'react';
import { getSupabaseBrowserClient } from '../lib/supabaseBrowserClient.js';
import { AuthShellThemeContext } from './AuthSplitShell.jsx';

function GoogleMark({ className }) {
  return (
    <svg className={className} viewBox="0 0 24 24" width="20" height="20" aria-hidden>
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}

/** Divider between OAuth and email/password — matches auth shell typography */
export function AuthEmailDivider() {
  const theme = useContext(AuthShellThemeContext);
  const isDark = theme === 'dark';
  return (
    <div className="relative py-1">
      <div
        className={`absolute inset-x-0 top-1/2 h-px -translate-y-1/2 ${
          isDark ? 'bg-slate-600/80' : 'bg-slate-200'
        }`}
        aria-hidden
      />
      <p
        className={`relative mx-auto w-max px-3 text-center text-[12px] font-semibold uppercase tracking-wide ${
          isDark ? 'bg-[#051120] text-slate-400' : 'bg-[#f0f4f8] text-slate-500'
        }`}
      >
        Or continue with email
      </p>
    </div>
  );
}

export function GoogleAuthButton({ label = 'Continue with Google' }) {
  const theme = useContext(AuthShellThemeContext);
  const isDark = theme === 'dark';
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState('');

  async function onClick() {
    setErr('');
    setBusy(true);
    try {
      const sb = await getSupabaseBrowserClient();
      const redirectTo = `${window.location.origin}/auth/callback`;
      const { error } = await sb.auth.signInWithOAuth({
        provider: 'google',
        options: { redirectTo }
      });
      if (error) throw error;
      // Browser redirects away; keep busy until unload
    } catch (e) {
      setErr(e?.message || 'Could not start Google sign-in');
      setBusy(false);
    }
  }

  return (
    <div className="flex flex-col gap-2">
      <button
        type="button"
        onClick={onClick}
        disabled={busy}
        className={`flex w-full items-center justify-center gap-3 rounded-xl border-2 py-3.5 text-[15px] font-bold transition-all disabled:opacity-60 ${
          isDark
            ? 'border-slate-500/60 bg-slate-800/80 text-slate-100 hover:bg-slate-700/90'
            : 'border-slate-200 bg-white text-slate-800 shadow-sm hover:bg-slate-50'
        }`}
      >
        <GoogleMark className="shrink-0" />
        {busy ? 'Redirecting…' : label}
      </button>
      {err ? (
        <div
          className={`rounded-lg px-3 py-2 text-center text-[13px] font-medium ${
            isDark ? 'bg-red-950/50 text-red-200 ring-1 ring-red-500/30' : 'bg-red-50 text-red-700 ring-1 ring-red-200'
          }`}
          role="alert"
        >
          {err}
        </div>
      ) : null}
    </div>
  );
}
